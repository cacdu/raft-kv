...
// tonic's generated gRPC service methods return `Result<_, tonic::Status>`,
// and `tonic::Status` is ~176 bytes, which trips clippy's `result_large_err`.
// That code is generated by `include_proto!` and can't be changed, so the lint
